...
echo "APIs"
[ $(curl --write-out %{http_code} --silent --output /dev/null -H "Accept: application/vnd.elife.annual-report-list+json; version=1" "${hostname}/annual-reports") == 200 ]
...
[ $(curl --write-out %{http_code} --silent --output /dev/null -H "Accept: application/vnd.elife.community-list+json; version=1" "${hostname}/community") == 200 ]
...
[ $(curl --write-out %{http_code} --silent --output /dev/null -H "Accept: application/vnd.elife.job-advert-list+json; version=1" "${hostname}/job-adverts") == 200 ]
...
[ $(curl --write-out %{http_code} --silent --output /dev/null -H "Accept: application/vnd.elife.press-package-list+json; version=1" "${hostname}/press-packages") == 200 ]
